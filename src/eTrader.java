...
public class eTrader {
	...
	private void incrementDayTradesTable(Stock s){
		DateFormat dateFormat = new SimpleDateFormat(
				"yyyy/MM/dd HH:mm:ss");
		Date date = new Date();
		String sql = "Insert into day_trades(Ticker_name, Buy_time, Sell_time) values ('"+s.getName()+"', "+s.getBuyTime()+", "+
				 dateFormat.format(date) + "');";
		this.conn.executeUpdate(sql);
	}
	
	private void cleanUpDayTradesTable(){//ideally, we only need to run this 1x a day.
		boolean oldTradesInTable=true;
		String sql = null;
		ResultSet oldest;
		while (oldTradesInTable){
			try {
				sql="Select Buy_time from Day_Trades order by Buy_time Limit 1";//get the oldest
				oldest=this.conn.executeQuery(sql);
				oldest.next();
				int businessDays=countBusinessDays(getDateFromString(oldest.getString(1)), new Date());
				if (businessDays>5){
					sql="Delete from Day_Trades where Buy_time='"+oldest.getString(1)+"';";
					this.conn.executeUpdate(sql);
				}
				else
